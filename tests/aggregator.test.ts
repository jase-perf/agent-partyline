...
describe('Aggregator', () => {
  ...
  test('SubagentStart records a subagent row with running status', () => {
    ...
    agg.ingest({
      ...
      ts: 't', payload: { description: 'find thing' },
  ...
  test('SubagentStop marks subagent completed with ended_at', () => {
    ...
    agg.ingest({
      machine_id: 'm', session_name: 'w', session_id: 's', hook_event: 'SubagentStart',
      ts: 't1', payload: {}, agent_id: 'a1', agent_type: 'Explore',
    })
    agg.ingest({
      machine_id: 'm', session_name: 'w', session_id: 's', hook_event: 'SubagentStop',
      ts: 't2', payload: {}, agent_id: 'a1',
    })
    const subs = agg.getSubagents('s')
  ...
  test('PostToolUse records a tool_call with success=1 by default', () => {
    ...
    const agg = new Aggregator(db)
    agg.ingest({
      machine_id: 'm', session_name: 'w', session_id: 's', hook_event: 'PostToolUse',
      ts: 't', payload: { tool_name: 'Bash', tool_response: { stdout: 'ok' } },
    })
    const rows = db
      .query<{ tool_name: string; success: number; session_id: string }, []>(
  ...
  test('PostToolUse with tool_response.success=false records failure', () => {
    ...
    agg.ingest({
      machine_id: 'm', session_name: 'w', session_id: 's', hook_event: 'PostToolUse',
      ts: 't', payload: { tool_name: 'Write', tool_response: { success: false, error: 'denied' } },
    })
    const rows = db
      .query<{ success: number }, []>('SELECT success FROM tool_calls')
      .all()
  ...
  test('PostToolUse with tool_response.isError=true records failure', () => {
    ...
    agg.ingest({
      machine_id: 'm', session_name: 'w', session_id: 's', hook_event: 'PostToolUse',
      ts: 't', payload: { tool_name: 'Read', tool_response: { isError: true, content: [] } },
    })
    const rows = db
      .query<{ success: number }, []>('SELECT success FROM tool_calls')
      .all()
  ...
  test('onUpdate listener fires with the updated session row', () => {
    ...
    agg.ingest({
      machine_id: 'm', session_name: 'w', session_id: 's', hook_event: 'SessionStart',
      ts: 't', payload: {},
    })
  ...
  test('listSessions returns sessions ordered by last_seen desc', () => {
    const db = openDb(DB)
    const agg = new Aggregator(db)
    agg.ingest({ machine_id: 'm', session_name: 'first', session_id: 's1', hook_event: 'SessionStart', ts: '2026-04-20T10:00:00Z', payload: {} })
    agg.ingest({ machine_id: 'm', session_name: 'second', session_id: 's2', hook_event: 'SessionStart', ts: '2026-04-20T11:00:00Z', payload: {} })
